...
function BadgeChip({ label, color = "violet" }: { label: string; color?: string }) {
...
function StatCard({ value, label, suffix, inView }: {
  value: string; label: string; suffix?: string; inView: boolean;
}) {
...
/* ─── Section heading ─────────────────────────────────────── */
function SectionHeading({ eyebrow, title, highlight }: {
      ...
      <div style={{ maxWidth: "80rem", margin: "0 auto" }}>
        ...
        <div className="about-grid" style={{
          ...
          <div>
            ...
            {/* Body text */}
            <motion.div {...staggerItem(0.1)} style={{
              ...
              {[
                <>I&apos;m a results-driven Software Development Engineer with strong hands-on
                  ...
                  <span style={{ color: T.t1 }}>React</span>, and cloud platforms.</>,
                <>Currently at{" "}
                  <span style={{ color: T.violet }}>Dr. D.Y. Patil School of Management</span>,
                ...
                <p key={i} style={{
                  ...
                  fontSize: 15, color: T.t2, lineHeight: 1.8, margin: 0,
                }}>{text}</p>
              ))}
            </motion.div>

            {/* Badges */}
            <motion.div {...staggerItem(0.2)} style={{
            ...
            {/* Education cards */}
            <motion.div {...staggerItem(0.3)} style={{
              ...
              {education.map(edu => (
                <div key={edu.id} style={{
                  ...
                  <div>
                    <div style={{
                      fontFamily: "'Syne', sans-serif",
                      fontWeight: 600, fontSize: 13, color: T.t1, marginBottom: 3,
                    }}>{edu.degree}</div>
                    <div style={{
                      fontFamily: "'JetBrains Mono', monospace",
                      fontSize: 11, color: T.t3,
                    }}>{edu.institution} · {edu.score}</div>
                    {edu.achievement && (
                      <div style={{
                        fontFamily: "'JetBrains Mono', monospace",
                        fontSize: 11, color: T.amber, marginTop: 2,
                      }}>{edu.achievement}</div>
                    )}
                  </div>
                </div>
          ...
          <div ref={ref}>
            ...
            >
              ...
              <div>
                <div style={{
                  fontFamily: "'Syne', sans-serif",
                  fontWeight: 700, fontSize: 14, color: T.emerald, marginBottom: 2,
                }}>Available for Opportunities</div>
                <div style={{
                  fontFamily: "'JetBrains Mono', monospace",
                  fontSize: 11, color: T.t3,
                }}>Pune, Maharashtra · Open to remote</div>
              </div>
            </motion.div>
          </div>
        </div>
